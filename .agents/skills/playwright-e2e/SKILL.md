---
name: playwright-e2e
description: "Playwright end-to-end testing skills for playwright-ts-template: plan, generate, review and debug tests with page objects, web-first assertions, fixtures and a test data strategy. Use when writing, fixing or reviewing Playwright tests, page objects, fixtures or playwright.config."
metadata:
...
│  ├─ Locators/asserts  → references/locators-and-assertions.md
│  └─ Review when done  → references/test-review.md
│
├─ Debug a FAILING test
│  ├─ Diagnose & fix    → references/test-debugging.md
│  ├─ Agent debugging   → references/agent-debugging.md
...
- **Before writing tests:** read `references/test-planning.md`, explore the page (playwright-cli if installed), then write using `references/test-generation.md`
- **Before submitting PRs:** run the `references/test-review.md` checklist
- **When tests fail:** follow `references/test-debugging.md`, attach with `--debug=cli` (`references/agent-debugging.md`), classify the root cause, then fix the test or report the app bug
