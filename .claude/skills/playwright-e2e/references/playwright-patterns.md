...
```typescript
const responsePromise = page.waitForResponse((resp) => resp.url().includes('/api/seats') && resp.status() === 200);
...
await expect(async () => {
  ...
  await expect(page.getByTestId('confirmation')).toBeVisible({ timeout: 1_000 });
  await expect(page.getByTestId('order-id')).not.toBeEmpty({ timeout: 1_000 });
}).toPass({
  timeout: 30_000,
...
const [seatsResponse, pricingResponse] = await Promise.all([seatsPromise, pricingPromise]);
```
